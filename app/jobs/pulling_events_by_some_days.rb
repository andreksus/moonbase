class PullingEventsBySomeDays < ApplicationJob
  ...
  def self.pulling_process(open_sea, end_date = nil, asset = nil, event_type = 'successful')
    ...
    if duplicate_count != max_api_items
      while !opensea_response.nil? && opensea_response[:next].present? || iteration_count == 1
        ...
        iteration_count += 1
        sleep 5
        Rails.logger.info "Event Type: #{event_type}, iteration_count = #{iteration_count}, duplicate_count = #{duplicate_count}, DateTime interation: #{ DateTime.parse(opensea_response[:asset_events].last[:created_date]).utc}"

        break if (duplicate_count == max_api_items) || (iteration_count == max_iteration) || DateTime.parse(opensea_response[:asset_events].last[:created_date]).utc <= (DateTime.parse("2022-07-28T15:59:05.958032") - 2.hour).utc
      end
    end
  end
